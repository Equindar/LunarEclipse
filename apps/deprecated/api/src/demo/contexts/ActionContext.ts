...
export class ActionContext implements IActionContext {
  ...
  build(params: {
    ...
    if (!action)
      throw new Error(`build ActionContext: no action entry for actor ${actorId} at index ${idx}`);
    ...
    selector(roundCtx, actorId, action).forEach((item) => {
      ...
      this.selectedTargets
        ? this.selectedTargets.push(newTarget)
        : (this.selectedTargets = [newTarget]);
    });
  ...
  execute(perspective: ActionPhase, registry?: RuleRegistry) {
    ...
    logger.debug(
      `${this.actor.id} benutzt: ${base.type} [t${this.actor.action.investedTempo},i${this.actor.action.investedImpact}] als [${perspective}]`,
    );

...
export const CriticalStrikeRule: Rule = {
  ...
  matches: (ctx) => {
    ...
    return (
      ctx.actionContext.actor.action.investedTempo >= 5 &&
      ctx.actionContext.actor.action.action.type === ActionType.ATTACK
    );
  },
  apply: (ctx) => {
    ...
    logger.debug(
      `"${CriticalStrikeRule.name}"-Rule angewendet. Angreifer erzielt Kritischen Treffer.`,
    );
  },
};

...
export class CombatEngine {
  ...
  public resolveRound(ctx: RoundContext): RoundContext {

    var attacker = ctx.self;
    var defender = ctx.target;
    logger.info(`Runde #${ctx.roundNumber} (Start): ${attacker.character.name}[HP:${attacker.character.hp},E:${attacker.character.energy}] vs ${defender.character.name}[HP:${defender.character.hp},E:${defender.character.energy}]`);
    ...
    this.registry.applyAll(ctx);

    logger.info(`Runde #${ctx.roundNumber} (End): ${attacker.character.name}[HP:${attacker.character.hp},E:${attacker.character.energy}] vs ${defender.character.name}[HP:${defender.character.hp},E:${defender.character.energy}]`);

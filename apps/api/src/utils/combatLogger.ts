...
export class CombatLogger {
  ...
  public log(msg: string) {
    const ms =
      this.combatCtx?.time.elapsed ??
      (this.combatCtx?.time.start ? Date.now() - this.combatCtx.time.start.getTime() : 0);
  ...
  public logAction(
    ac: IActionContext | undefined,
    msg: string,
    level: keyof winston.Logger['levels'] = 'action',
  ) {
    if (!ac) {
      this.log(msg);
      return;
    ...
    const actionName =
      (ac.actor.action && (ac.actor.action.constructor?.name ?? ac.actor.action.action.type)) ??
      'Action';

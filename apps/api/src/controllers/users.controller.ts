...
export default class UsersController {
  ...
  public onCreateUser = async (
    req: Request<{}, {}, CreateUserDTO>,
    res: Response,
    next: NextFunction,
  ) => {
    try {
      await new createUser(new UserRepositoryImpl(new UserDataSourceImpl(this.database))).execute(
        req.body,
      );
  ...
  public onGetUser = async (
    req: Request<GetUserDTO, {}, GetUserDTO>,
    res: Response,
    next: NextFunction,
  ) => {
    const data = await new getUser(
      new UserRepositoryImpl(new UserDataSourceImpl(this.database)),
    ).execute(req.params);
    return res.status(200).send(UserDTO.fromEntity(data!));
  };

  public onListUsers = async (req: Request, res: Response, next: NextFunction) => {
    const data = await new listUsers(
      new UserRepositoryImpl(new UserDataSourceImpl(this.database)),
    ).execute();
    const result: UserDTO[] = [];
    data.forEach((item) => {
      result.push(UserDTO.fromEntity(item));
    });

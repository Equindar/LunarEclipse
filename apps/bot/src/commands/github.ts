...
let command: Command = {
    ...
    .addSubcommand((subcommand) =>
      subcommand
        ...
        .addNumberOption((option) =>
          option.setName('id').setDescription('Nummer des Issues').setMinValue(0).setRequired(true),
        )
  ...
  async execute(interaction) {
    try {
      ...
      const images: { media: { url: string } }[] = urls.map((item) => ({
        media: { url: item[1] },
      }));
      const clearedContentBody = content.body.replace(
        /<img[^>]*src\s*=\s*['"][^'"]+['"][^>]*>/g,
        '',
      );
      ...
      if (content.created_at != content.updated_at) {
        ...
        containerComponent.addTextDisplayComponents(
          new TextDisplayBuilder().setContent(
            `-# __Zuletzt geändert:__ ${formatDateTime(content.updated_at)}`,
          ),
        );
      }

...
export const characters = mysqlTable(
  'characters',
  {
    id: int('ID')
      ...
      .references(() => worlds.id, { onDelete: 'restrict', onUpdate: 'restrict' }),
  ...
  (table) => [unique('characters_unique').on(table.name)],
);
...
export const characterTraits = mysqlTable('character_traits', {
  id: int('ID')
    ...
    .references(() => traits.id, { onDelete: 'restrict', onUpdate: 'restrict' }),
...
export const items = mysqlTable(
  'items',
  {
    id: int('ID')
      .autoincrement()
      .notNull()
      .references(() => rarity.id, { onDelete: 'restrict', onUpdate: 'restrict' }),
  ...
  (table) => [unique('items_unique').on(table.name)],
);
...
export const traits = mysqlTable(
  'traits',
  {
  ...
  (table) => [unique('traits_unique').on(table.name)],
);
...
export const users = mysqlTable(
  ...
  (table) => [unique('Users_unique').on(table.username)],
);
...
export const worlds = mysqlTable(
  ...
  (table) => [unique('worlds_unique').on(table.name)],
);

export const rarity = mysqlTable(
  '_rarity',
  {
    id: int('ID').autoincrement().notNull(),
    name: varchar('Name', { length: 100 }).notNull(),
  ...
  (table) => [unique('_rarity_unique').on(table.name)],
);

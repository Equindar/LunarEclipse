import { mysqlTable, mysqlSchema, AnyMySqlColumn, foreignKey, unique, int, varchar, timestamp, index, check, longtext, mysqlEnum, text, datetime } from "drizzle-orm/mysql-core"
...
export const charactersInventory = mysqlTable("characters_inventory", {
  id: int("ID").autoincrement().notNull().references(() => itemsVersion.id, { onDelete: "restrict", onUpdate: "restrict" }),
...
export const usersLogins = mysqlTable("users_logins", {
  id: int("ID").autoincrement().notNull(),
  userId: int("User_ID").notNull().references(() => users.id, { onDelete: "restrict", onUpdate: "restrict" }),

...
export class User {
  ...
  static register(nickname: string): User {
    return new User(ulid(), nickname, new InitializedUserStatus(), new Date());
  }

  static get(id: number, uuid: ULID, nickname: string, createdDate: Date): User {
    const user = new User(uuid, nickname, new InitializedUserStatus(), createdDate);
    user.id = id;
    return user;
  }

  static edit(id: number, uuid: ULID, nickname: string, createdDate: Date): User {
    const user = new User(ulid(), nickname, new InitializedUserStatus(), new Date());
    user.id = id;
    return user;
  }
